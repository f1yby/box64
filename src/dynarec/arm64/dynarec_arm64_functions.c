...
void inst_name_pass3(dynarec_native_t* dyn, int ninst, const char* name, rex_t rex)
{
    if(box64_dynarec_dump) {
        ...
        dynarec_log(LOG_NONE, "%s%p: %d emitted opcodes, inst=%d, barrier=%d state=%d/%d(%d), %s=%X/%X, use=%X, need=%X/%X, sm=%d(%d/%d), taint=%d/%d",
            ...
            dyn->insts[ninst].x64.taint,
            dyn->insts[ninst].x64.taint_touched);
